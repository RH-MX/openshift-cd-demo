...
function deploy() {
  ...
  local template=../cicd-template-offline.yaml
  echo "Using template $template"
  oc $ARG_OC_OPS create -f ../templates/offline/gogs-template.yaml -n cicd-$PRJ_SUFFIX
  oc $ARG_OC_OPS create -f ../templates/offline/gogs-persistent-template.yaml -n cicd-$PRJ_SUFFIX
  oc $ARG_OC_OPS create -f ../templates/offline/nexus3-template -n cicd-$PRJ_SUFFIX
  oc $ARG_OC_OPS create -f ../templates/offline/nexus3-persistent-template -n cicd-$PRJ_SUFFIX
  oc $ARG_OC_OPS create -f ../templates/offline/sonarqube-template -n cicd-$PRJ_SUFFIX
  oc $ARG_OC_OPS create -f ../templates/offline/sonarqube-persistent-template -n cicd-$PRJ_SUFFIX
  ...
  sleep 5
    ...
    cat <<EOF > data.json
    {
        "type": "gogs",
        "config": {
        "url": "https://$HOSTNAME/oapi/v1/namespaces/$CICD_NAMESPACE/buildconfigs/tasks-pipeline/webhooks/${WEBHOOK_SECRET}/generic",
        "content_type": "json"
        },
        "events": [
        "push"
    ...
    _RETURN=$(curl -o curl.log -sL -w "%{http_code}" -H "Content-Type: application/json" \
    -u $GOGS_USER:$GOGS_PWD -X POST http://$GOGS_HOSTNAME/api/v1/repos/gogs/openshift-tasks/hooks -d @data.json)

    if [ $_RETURN != "201" ] ; then
        echo "ERROR: Failed to set webhook ${WEBHOOK_SECRET}"
        cat /tmp/curl.log
        echo "Try to set webhook with hook.json and sethook.sh"#exit 255
    fi
}
...
echo_header "OpenShift CI/CD Tenant ($(date))"
